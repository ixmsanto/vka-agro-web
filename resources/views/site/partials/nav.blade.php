...
<nav id="vka-nav" style="position:fixed;top:0;left:0;right:0;z-index:100;background:transparent;border-bottom:1px solid transparent;backdrop-filter:blur(0px);-webkit-backdrop-filter:blur(0px);transition:background .45s ease, box-shadow .45s ease, border-color .45s ease, backdrop-filter .45s ease">
    ...
    <div id="vka-topbar" class="vka-desktop-block" style="background:#123C2D;color:rgba(255,255,255,0.82);overflow:hidden;transition:max-height .45s ease, opacity .35s ease;max-height:44px;opacity:1">
        <div style="max-width:1440px;margin:0 auto;padding:0 clamp(18px,3vw,44px);height:38px;display:flex;align-items:center;justify-content:space-between;gap:20px">
            <div style="display:flex;align-items:center;gap:22px;font-size:12.5px;font-weight:500">
                <a href="mailto:{{ $contact['email'] }}" class="vka-link-muted" style="display:inline-flex;align-items:center;gap:8px;color:rgba(255,255,255,0.82)">
                ...
                <span style="display:inline-flex;align-items:center;gap:8px;color:rgba(255,255,255,0.62)"><span style="width:5px;height:5px;border-radius:50%;background:#63BE46"></span>Mon–Sat · 9:00–18:00 IST</span>
            </div>
            <div style="display:flex;align-items:center;gap:16px">
                <span style="font-size:11.5px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:rgba(255,255,255,0.55)">ISO 9001:2015 · Exporting to 40+ countries</span>
    ...
    <div style="max-width:1440px;margin:0 auto;padding:0 clamp(18px,3vw,44px);height:clamp(66px,7vw,80px);display:flex;align-items:center;justify-content:space-between;gap:clamp(14px,2vw,32px)">
        <a href="#hero" aria-label="VKA Agro Products home" style="display:flex;align-items:center;flex:0 0 auto">
            ...
            @else
                <span style="font-family:'Newsreader',serif;font-size:clamp(26px,2.6vw,32px);line-height:1;color:#123C2D">VKA<span style="font-style:italic;color:#63BE46">&nbsp;Agro</span></span>
            @endif
        </a>
        ...
        <div class="vka-desktop" style="align-items:center;gap:clamp(12px,1.4vw,20px);flex:0 0 auto">
            <a href="tel:{{ $contact['phoneHref'] }}" class="vka-wide" style="align-items:center;gap:10px">
                ...
                <span style="display:flex;flex-direction:column;line-height:1.15">
                    <span style="font-size:14px;font-weight:700;color:#123C2D">{{ $contact['phone'] }}</span>
                    <span style="font-size:11px;color:#8A968E">Mon–Sat · 9–6</span>
                </span>
            </a>
            <a href="#contact" class="vka-btn-primary" style="display:inline-flex;align-items:center;gap:9px;background:#2F8B3C;color:#FFFFFF;padding:12px 22px;border-radius:999px;font-size:14.5px;font-weight:600;white-space:nowrap;box-shadow:0 8px 22px rgba(47,139,60,0.22)">Get a Quote <span aria-hidden="true" style="display:inline-flex;width:21px;height:21px;border-radius:50%;background:rgba(255,255,255,0.2);align-items:center;justify-content:center">→</span></a>
        </div>

        <button id="vka-burger" class="vka-mobile" type="button" aria-label="Toggle menu" aria-expanded="false" aria-controls="vka-drawer" style="flex:0 0 auto;background:rgba(255,255,255,0.85);border:1px solid #E1EFDD;border-radius:14px;width:46px;height:46px;padding:0;flex-direction:column;align-items:center;justify-content:center;gap:5px;cursor:pointer;box-shadow:0 4px 14px rgba(33,80,60,0.08)">
...
<div id="vka-drawer" style="position:fixed;inset:0;z-index:99;background:rgba(250,253,248,0.98);backdrop-filter:blur(24px);-webkit-backdrop-filter:blur(24px);padding:clamp(84px,16vw,110px) clamp(24px,7vw,52px) 40px;flex-direction:column">
    <div style="display:flex;flex-direction:column">
        @foreach ($links as $href => $label)
            <a href="{{ $href }}" style="font-family:'Newsreader',serif;font-size:clamp(28px,8vw,38px);color:#123C2D;padding:clamp(12px,2.4vw,16px) 0;border-bottom:1px solid #E1EFDD">{{ $label }}</a>
        @endforeach
    </div>
    <a href="#contact" style="display:inline-flex;align-self:flex-start;align-items:center;gap:10px;margin-top:clamp(24px,5vw,34px);background:#2F8B3C;color:#FFFFFF;padding:16px 30px;border-radius:999px;font-size:16px;font-weight:600;box-shadow:0 14px 32px rgba(47,139,60,0.3)">Request a Quote <span aria-hidden="true">→</span></a>
    <a href="tel:{{ $contact['phoneHref'] }}" style="margin-top:22px;font-size:15px;font-weight:600;color:#5E6862">{{ $contact['phone'] }} · Mon–Sat 9–6</a>
</div>

...
<section id="why" style="position:relative;padding:clamp(64px,9vw,120px) 0;scroll-margin-top:80px">
    <x-deco shape="leaf" size="32" pos="top:8%;left:4%" opacity="0.34" motion="drift" rotate="-16" wide />
    <x-deco shape="dots" size="44" pos="top:11%;right:3.5%" opacity="0.25" motion="float-slow" delay="1.5s" wide />
    <x-deco shape="drop" size="28" pos="bottom:9%;left:3%" opacity="0.32" motion="float" delay="2.3s" wide />
    <x-deco shape="husk" size="50" pos="bottom:24%;right:5.5%" opacity="0.21" motion="spin" wide />
    ...
    <div style="max-width:1400px;margin:0 auto;padding:0 clamp(20px,4vw,48px);position:relative;z-index:1">
        <div style="text-align:center;max-width:660px;margin:0 auto clamp(40px,5vw,60px)">
            ...
            <h2 data-split="1" style="font-family:'Newsreader',serif;font-weight:400;font-size:clamp(34px,4.4vw,60px);line-height:1.06;letter-spacing:-0.015em;margin:18px 0 0;color:#123C2D">A supplier your <span style="font-style:italic;color:#63BE46">agronomist</span> can trust.</h2>
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(min(300px,100%),1fr));gap:clamp(16px,2vw,24px)">
            @foreach ($cards as $i => [$title, $body, $icon])
                <div data-reveal="{{ $i + 3 }}" data-fx="scale" data-tilt="7" style="background:#FFFFFF;border:1px solid #EEF3EC;border-radius:22px;padding:30px 28px;box-shadow:0 16px 44px rgba(33,80,60,0.07)">
                    <span data-ico style="width:52px;height:52px;border-radius:15px;background:#EDF8EC;display:inline-flex;align-items:center;justify-content:center;color:#2F8B3C"><svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round">{!! $icon !!}</svg></span>
